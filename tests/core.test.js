...
test('sanitizeStore always produces all four tabs', () => {
  const out = T.sanitizeStore({ cardio: { '2026-05-01': { mins: 30 } } });
  assert.deepEqual(Object.keys(out).sort(), ['cardio', 'intervals', 'lifting', 'weight']);
...
test('sanitizeStore handles null / undefined input', () => {
  ...
  assert.deepEqual(out, { cardio: {}, weight: {}, lifting: {}, intervals: {} });
});
